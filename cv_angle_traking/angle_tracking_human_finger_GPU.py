# cvtcolor and bluring are accelerated with GPU
import time,os
...
class TrackHumanFinger(object): # TODO
    ...
    def __init__(self, video_name=[],denoising_mode='monocolor'): # TODO
        ...
        self.num_marker_sets = 5
        self.denoising_mode = denoising_mode# 'monocolor'
        self.colors = [(255,0,0), (127,0,255), (0,127,0),(0,127,255), (255,255,255)]

        if self.color_mode ==0: # Lab
            self.maker_tolerance_L = [10,15,15,10,8]#int(0.08 * 255)
            self.maker_tolerance_a = [10,5,15,15,15]# int(0.09 * 255)# red -> green
            self.maker_tolerance_b = [10,5,15,15,15]# int(0.09 * 255)# Yellow -> Blue
        else : # RGB
        ...
        self.gauss_filter = cv2.cuda.createGaussianFilter(
    ...
    def calculate_angle(self, marker_pos): # marker_pos = [(fingertip),(DIP),(PIP),(MCP),(palm_marker)]
        # angles = []
        # for i in range(3):
        #     distal = np.array(joints_pos[i])
        #     middle = np.array(joints_pos[i+1])
        #     proximal = np.array(joints_pos[i+2])
        #     distal_vec = distal - middle
        #     proximal_vec = middle - proximal

        #     dot_product = np.dot(proximal_vec, distal_vec)
        #     cross_product = np.cross(proximal_vec, distal_vec)
        #     # norm_distalis = np.linalg.norm(distal_vec)
        ...
        for i in range(3):
            # 座標をアンパックしてローカル変数に
            x0, y0 = marker_pos[i]
            x1, y1 = marker_pos[i+1]
            x2, y2 = marker_pos[i+2]

            # ベクトル proximal = (PIP→MCP), distal = (DIP→PIP)
            px, py = x1 - x2, y1 - y2
            dx, dy = x0 - x1, y0 - y1
    ...
    def segment_marker_by_color(self,frame_tmp): # Askar.L
        ...
        if True: # Display masks in a cv high gui window
            mask_in_one = np.vstack((markers_masks[0],markers_masks[1],markers_masks[2],markers_masks[3],markers_masks[4]))
    ...
    def acquire_marker_color(self, frame, cv_choose_wd_name): #TODO
        ...
        self.fingertip = self.marker_position_frame0[0]
        self.DIP = self.marker_position_frame0[1]
        self.PIP = self.marker_position_frame0[2]
        self.MCP = self.marker_position_frame0[3]
        self.palm = self.marker_position_frame0[4]

        print('marker position frame0', self.marker_position_frame0)
    ...
    def mouse_event(self,event, x, y, flags, param):
        ...
        return []
    def segment_marker_by_color_gpu(self, frame, gauss_filter): # by nagamine, including cvtcolor process!

        self.gpu_frame.upload(frame)
        self.gpu_lab = cv2.cuda.cvtColor(self.gpu_frame, cv2.COLOR_RGB2Lab)
        self.gpu_blur = gauss_filter.apply(self.gpu_lab)

        # gpu_mask_1 = cv2.cuda.inRange(gpu_blur, tuple((self.marker_rangers[0]).T[0]), tuple((self.marker_rangers[0]).T[1]))
        # gpu_mask_2 = cv2.cuda.inRange(gpu_blur, tuple((self.marker_rangers[1]).T[0]), tuple((self.marker_rangers[1]).T[1]))
        # gpu_mask_3 = cv2.cuda.inRange(gpu_blur, tuple((self.marker_rangers[2]).T[0]), tuple((self.marker_rangers[2]).T[1]))
        # gpu_mask_4 = cv2.cuda.inRange(gpu_blur, tuple((self.marker_rangers[3]).T[0]), tuple((self.marker_rangers[3]).T[1]))
        # gpu_mask_5 = cv2.cuda.inRange(gpu_blur, tuple((self.marker_rangers[4]).T[0]), tuple((self.marker_rangers[4]).T[1]))

        # mask1,mask2,mask3,mask4,mask5 = gpu_mask_1.download(), gpu_mask_2.download(), gpu_mask_3.download(), gpu_mask_4.download(), gpu_mask_5.download(), 
        # masks = np.stack([mask1,mask2,mask3,mask4,mask5], axis=0)
        # return masks
        gpu_masks = []
        for rng in self.marker_rangers:  
            ...
            self.gpu_mask.create(self.gpu_blur.size(), cv2.CV_8UC1)

            cv2.cuda.inRange(self.gpu_blur, lowerb, upperb, self.gpu_mask)
            gpu_masks.append(self.gpu_mask.download())


        masks = np.stack(gpu_masks, axis=0)  # shape=(5, H, W)
        del gpu_masks
    ...
    def extract_angle(self, frame, swap, colors):
        ...
        masks = self.segment_marker_by_color_gpu(frame,self.gauss_filter)
        ...
        if 1:
            ...
            for mask, thr, color, color_name,  color_num in zip(masks, threshold_area_size, colors, colors_name, [0,1,2,3,4]):
                ...
                makerset_per_frame.append(point_per_mask)
                joints_pos.append(point_per_mask[0])

                if color_num == 0:
                    self.fingertip = point_per_mask[0]
                elif color_num == 1:
                    self.DIP = point_per_mask[0]
                elif color_num == 2:
                    self.PIP = point_per_mask[0]
                elif color_num == 3:
                    self.MCP = point_per_mask[0]
                elif color_num == 4:
                    self.palm = point_per_mask[0]
            ...
            print('jointspos', joints_pos)

            angles = self.calculate_angle(joints_pos)

            _text_pos_x = 100
            # Add text annotations to the frame with calculated angles
            frame = self.add_text_to_frame(frame, "ANGLE 0: {}".format((angles[0])), position=(_text_pos_x, 210), font_scale=1, thickness=2, color=(255, 255, 0))
            frame = self.add_text_to_frame(frame, "ANGLE 1: {}".format((angles[1])), position=(_text_pos_x, 240), font_scale=1, thickness=2, color=(255, 255, 0))
            frame = self.add_text_to_frame(frame, "ANGLE 2: {}".format((angles[2])), position=(_text_pos_x, 270), font_scale=1, thickness=2, color=(255, 255, 0))
...
if __name__ == '__main__':
    ...
    # Constants
    ## For styling
    colors = [(43,74,134), (0,0,255), (255,0,0), (0,255,255),(255,255,255)]
    line_padding = [0.7, 1.5, 1.5, 1.5, 1.5]

    font_scale = 1
    text_position_cnt = (100, 100)
    text_position_time = (100, 120)
    
    video_name = "slowmo_test.mp4"
    frame_jump = 0

    ## For algorithm tuning
    # Are for optime
    kernel = np.ones((5,5),np.uint8)
    threshold_area_size = [50, 50, 50, 10, 50]# [80, 20, 10, 40]
    frame_shift = 0
    output_video_fps = 240

    tracker = TrackHumanFinger(video_name,denoising_mode = 'monocolor')
    # Main logic
    ...
    try:
        while True:
            ...
            frame, angles  = tracker.extract_angle(frame,False, colors)
            angle_0 = angles[0]
            angle_1 = angles[1]
            angle_2 = angles[2]
