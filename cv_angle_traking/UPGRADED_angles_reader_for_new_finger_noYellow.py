...
class AngleTracker(object): # TODO
    ...
    def acquire_marker_color(self,frame): #TODO
        ...
        self.metacarpal_line = self.marker_position_frame0.pop()
        ...
        if 1:
            ...
            for mask, thr, color, color_name, direction_vector in zip(masks, self.threshold_area_size, self.colors, colors_name, [direction_vector_0_1, None, None, None]        ):
                ...
                for idx, index in enumerate(filtered_regions):
                    ...
                    #     point_per_mask.append((centroid_x + 100, centroid_y))
            ...
            self.temp_yellow_y = self.metacarpal_line[1]
            metacarpal = []
            metacarpal.append([self.metacarpal_line[0]-1000, self.metacarpal_line[1]])
            metacarpal.append([self.metacarpal_line[0]+1000, self.metacarpal_line[1]]) 
            ...
            angle_0 = self.calculate_angle(markerset_per_frame[0], markerset_per_frame[1], 0)
            angle_1 = self.calculate_angle(markerset_per_frame[1], markerset_per_frame[2], 1)
            # angle_2 = self.calculate_angle(markerset_per_frame[2], markerset_per_frame[3], 2)
            angle_2 = self.calculate_angle(markerset_per_frame[2], metacarpal, 2)
            
            #Find angles position by calculating intersections of lines 
            if calc_intersection:
                angle0_pos = self.calculate_intersection(markerset_per_frame[0], markerset_per_frame[1])
                angle1_pos = self.calculate_intersection(markerset_per_frame[1], markerset_per_frame[2])
                angle2_pos = self.calculate_intersection(markerset_per_frame[2], metacarpal)
                self.b0 = blue_pos[0]
                self.b1 = blue_pos[1]
