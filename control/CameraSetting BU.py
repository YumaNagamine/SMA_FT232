...
class Camera(VideoCapture):
    ...
    def __init__(self,CAM_NUM, CAP_API,cam_name="side",video_path = None):
        super().__init__(CAM_NUM, CAP_API)
    ...
    def load_calibration(self, config_path: str = CAM_CAL_SIDE_URL):
        """
        Load and normalize flat-field reference at IMG/<mode>cali/flat_white.jpg.
        """
        flat_path = f'./IMG/{self.cam_name}cali/flat_white3.jpg'
    ...
    def process_frame(self, frame,
        ...
        # 2) Clip & convert back to uint8 before undistort
    ...
    def realtime(self): #Refined in 250519
        ...
        if cam_name == 'AR0234': # Aptina AR0234
            ...
            manual_settings = {
                ...
                cv2.CAP_PROP_AUTO_WB:             1,      # disable auto white balance
                ...
                cv2.CAP_PROP_BRIGHTNESS:          1,    # mid‐point
                cv2.CAP_PROP_CONTRAST:            8,
    ...
    def existingvideo(self, video_path, folloing_name = 'processed', frame_shift = 0, output_fps=90):
        self.set(cv2.CAP_PROP_POS_FRAMES, frame_shift)
        self.set(cv2.CAP_PROP_FPS, output_fps)
        self.output_path = video_path + '_' + folloing_name      
...
if __name__ == '__main__': #   Quick Test the Camera class
    ...
    # === before your while-loop, after cv2.namedWindow & cv2.resizeWindow ===
    # Create a little control panel window
    window_name = 'Original | Processed'
    # 1) make window resizable
    cv2.namedWindow(window_name, cv2.WINDOW_NORMAL)
    # 2) move it to top-left corner (0,0)
    cv2.moveWindow(window_name, 0, 0)
    # 3) resize window to half of 4K width (i.e. 1920×1080)
    #    Adjust these numbers if you want a different size.
    cv2.resizeWindow(window_name, 1920, 1080)
    ctrl_win = 'Controls'
    cv2.namedWindow(ctrl_win, cv2.WINDOW_AUTOSIZE)

    # # Trackbars for morphology
    # cv2.createTrackbar('Kernel', ctrl_win, cam.morphology['kernel_size'], 50, nothing)
    # cv2.createTrackbar('Blur',   ctrl_win, cam.morphology['blur_size'],   50, nothing)

    # Toggle switch: 0 = OFF, 1 = ON
    cv2.createTrackbar('Process ON/OFF', ctrl_win, 1, 1, nothing)


    # === then your capture loop ===
    while True:
        ret, frame = cam.read()
        if not ret:
            continue
        ...
        # read UI controls
        k = cv2.getTrackbarPos('Kernel', ctrl_win)
        b = cv2.getTrackbarPos('Blur',   ctrl_win)
        sw = cv2.getTrackbarPos('Process ON/OFF', ctrl_win)

        # enforce odd sizes
        k = max(1, k//2*2+1)
        b = max(1, b//2*2+1)

        # # update cam morphology
        # cam.morphology['kernel_size'] = k
        # cam.morphology['blur_size']   = b
