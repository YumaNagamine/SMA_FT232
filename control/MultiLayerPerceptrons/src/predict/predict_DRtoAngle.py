...
# 1. モデル定義を同じ形で用意
model = DutyRatioNet(input_dim=10, output_dim=4, hidden_layer_size=[128, 64, 32])

# 2. 保存したパラメータをロード
checkpoint = torch.load("./control/MultiLayerPerceptrons/checkpoints_DRtoAngle/model_epoch_50.pth", map_location="cpu")
...
predict_ds = DutyratioToAngleDataset("./control/MultiLayerPerceptrons/src/predict/FDP_predict_data.csv", noise_std=0.0)
...
with torch.no_grad():
    for inputs, _ in predict_loader:   # ラベル y が不要なら "_" で受け取る
        outputs = model(inputs)        # (batch_size, 4) のテンソル
