# Debugged @ 20241221
# Created by Nagmine
# Altering NOGUI_ASYNCSAVER_with_ANGLESREADER so that it could be used in REALTIME CONTROL
...
class AngleTracker(AsyncVideoSaver):
    def __init__(self, filename, fourcc, fps, frame_size, denoising_mode='monocolor'):
        ...
        self.threshold_area_size = [50, 30, 50]
        self.colors = [(0,127,0), (127,0,255), (255,0,0)] 

        self.video_pos_file_url = "C:/Users/SAWALAB/Desktop/projects/SMA_FT232/LOG/realtime/pos.json"

        if self.color_mode == 0: # Lab
            self.marker_tolerance_L = [20,40,15]#int(0.08 * 255)q
            self.marker_tolerance_a = [20,45,15]# int(0.09 * 255)# red -> green
            self.marker_tolerance_b = [20,20,15]# int(0.09 * 255)# Yellow -> Blue
        else : # RGB
        ...
        self.enable_marker_pos_acquirement = False
        self.load_point_pos()
        self.angle_pos = []
    ...
    def segment_marker_by_color(self,frame_tmp): # Askar.L # used in extract_angle
        ...
        for i in range(num_marker_sets):# Color segmentation using NumPy array operations
    ...
    def acquire_marker_color(self): #TODO
        ...
        if self.enable_marker_pos_acquirement:
            ...
            _meassage = "Loaded exsist marker position:"
        ...
        for _i,_pos in enumerate(self.marker_position_frame0):
            ...
            upper_limit = frame_to_segment[_pos[1]][_pos[0]] + [
                self.marker_tolerance_L[_i], self.marker_tolerance_a[_i], self.marker_tolerance_b[_i]]  
    ...
    def extract_angle(self):
        ...
        [marker_blue, marker_pink, marker_green] = self.segment_marker_by_color(cielab_frame)


        # Create a stack of masks for each color marker
        masks = np.stack([marker_green, marker_pink, marker_blue], axis=0)

        # Define color names for visualization
        colors_name = ["green", "pink", "blue"]
        ...
        if 1:
            ...
            for mask_index ,mask, thr, color, color_name, direction_vector in zip([0,1,2], masks, self.threshold_area_size, self.colors, colors_name, [direction_vector_0_1, None, None, None]        ):
                ...
                for idx, index in enumerate(filtered_regions): # idealy, filtered regions have one element
                    ...
                    if mask_index == 0: #purple. Estimate PIP using MCP and purple marker
                        ...
                        if direction_vector is None:
                            direction_vector = self.calculate_vector(self.MCP_pos, point_per_mask[0])

                        # Calculate points for the line based on the direction vector and line padding
                        point1 = (int(self.MCP_pos[0] - line_pad * direction_vector[0]),
                                int(self.MCP_pos[1] - line_pad * direction_vector[1]), )

                        # Visualize the line connecting the two points
                        cv2.line(self.frame, point1, self.PIP_pos, color, 3)
                    ...
                    if mask_index == 1: #red. Estimate DIP using PIP and red marker
                        ...
                        if direction_vector is None:
                            direction_vector = self.calculate_vector(self.PIP_pos, point_per_mask[0])
                        point1 = (int(self.PIP_pos[0] - line_pad * direction_vector[0]),
                                int(self.PIP_pos[1] - line_pad * direction_vector[1]), )
                        cv2.line(self.frame, point1, self.DIP_pos, color, 3)
                    ...
                    if mask_index == 2: #blue. Estimate Fingertip using DIP and blue marker
                        ...
                        if direction_vector is None:
                            direction_vector = self.calculate_vector(self.DIP_pos, point_per_mask[0])
                        point1 = (int(self.DIP_pos[0] - line_pad * direction_vector[0]),
                                int(self.DIP_pos[1] - line_pad * direction_vector[1]), )
                        cv2.line(self.frame, point1, self.fingertip_pos, color, 3)
                ...
                for idx, point in enumerate(point_per_mask):
                    cv2.circle(self.frame, (point[0], point[1]), radius=(idx+1) * 10, color=color, thickness=2)
            ...
            # create variable of metacarpal line
            self.temp_yellow_y = self.MCP_pos[1]
            metacarpal = []
            metacarpal.append([self.MCP_pos[0], self.MCP_pos[1]])
            metacarpal.append([self.MCP_pos[0]+1000, self.MCP_pos[1]]) 
            cv2.line(self.frame, metacarpal[0], metacarpal[1],(0,127,255),3)

            # Calculate angles between consecutive lines
            # print(markerset_per_frame)
    ...
    def mouse_event(self, event, x, y, flags, param):
        ...
        if event == cv2.EVENT_RBUTTONDOWN:
            ...
            cv2.putText(self.frame, _meassage, (50, 200), cv2.FONT_HERSHEY_PLAIN, 2, (255, 255, 255), thickness = 1)
            self.enable_marker_pos_acquirement = True
            pass
    ...
    def load_point_pos(self,):
        ...
        try:
            ...
            print("\tErr occurs when loading marker_position_frame0, Please calibrate angle sensor: \n",Err)
            # self.calibrateRange()
        pass
    ...
    def store_point_pos(self,):
        ...
        _data = {'marker_position_frame0':_data}
